            ...
            if not marketdata.ib.isConnected():
                ...
                try:
                    ...
                    for row in zombie_exit_rows:
                        ...
                        log.warning(
                            "Post-reconnect zombie purge: emitting TRADE_CLOSED for filled record "
                            "%s signal_id=%s (was open, now untracked)",
                            row.get("symbol"), row.get("signal_id"),
                        )
                        print(f"[ZOMBIE_PURGE] TRADE_CLOSED emitted: {row.get('symbol')} "
                              f"{row.get('signal_id')} fill={row.get('fill_price')}")
                    if purged:
