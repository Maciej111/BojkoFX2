...
class ExecutionEngine:
    ...
    def _close_position(self, trade, price, time, reason):
        ...
        if trade.risk_distance > 0:
            ...
            raise ValueError(f"Trade {trade.id}: risk_distance must be > 0, got {trade.risk_distance}")
        ...
        # Commission
        # commission_per_lot per round turn?
        # Usually total commission.
        comm = self.config['commission_per_lot'] # assuming 1 lot fixed for simplicity
        # or scale by lot size? Let's assume lot_size is 1 standard lot (100k).
        # Actually user config says lot_size: 100000.
        # So comm is $7.
